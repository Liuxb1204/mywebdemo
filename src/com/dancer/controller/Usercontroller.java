...
public class Usercontroller {
	...
	public String userlogin(User user,String image,HttpSession session,HttpServletRequest request){
		String ma=(String) session.getAttribute("validateCode");
		User byName = userservice.queryName(user.getUsername());
		if(byName==null || !user.getPassword().equals(byName.getPassword())){
			return "login";
		}
		else if(!ma.equals(image)){
			System.out.println(ma+":验证码错误"+"image:"+image);
...
//	}
	...
	public @ResponseBody String selectByName(String username){
		User queryName = userservice.queryName(username);
		if(queryName==null){
			return "null";
		}else{
	...
	public @ResponseBody String userZhuce(User user,String password1){
		...
		if(userservice.queryName(user.getUsername())==null){
