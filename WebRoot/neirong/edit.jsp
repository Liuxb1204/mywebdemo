...
<head>
  ...
  function tijiao(){
  		$('#Form').form('submit', {   
	   			 url:'${pageContext.request.contextPath }/wenzhang/update.do',   
			   	 onSubmit: function(){   
			      return true; 
			    },   
...
<body>
	<form id="Form" action="" method="post"
		...
		<table class="input tabContent" style="display: table;" >
			<tbody>
				<tr style="display:none">
					...
					<td><input type="text" name="id" value="${requestScope.wz.id}"
						class="text" ></td>
				</tr>
				<tr>
					<th><span class="requiredField">*</span>标题:</th>
					<td><input type="text" name="biaoti" value="${requestScope.wz.biaoti}"
						class="text" ></td>
				</tr>
				<tr>
					<th><span class="requiredField">*</span>副标题:</th>
					<td><input type="text" name="twobiaoti" class="text" value="${requestScope.wz.twobiaoti}" >
					</td>
				</tr>
				<tr>
					<th>作者:</th>
					<td><input type="text" name="whiter" class="text" value="${requestScope.wz.whiter}">
					</td>
				</tr>
				<tr>
					<th>文章类型:</th>
				...
				<tr>
					...
					<td>
						<input type="text" name="chucu" class="text" value="${requestScope.wz.chucu}">
					</td>
				</tr>
				<tr>
					<th><span class="requiredField">*</span>链接:</th>
					<td>
						<input type="text" name="link" class="text" value="${requestScope.wz.link}">
					</td>
				</tr>
				<tr>
					<th><span class="requiredField">*</span>商家:</th>
					<td>
						<input type="text" name="sellername" class="text" value="${requestScope.wz.sellername}">
					</td>
				</tr>
				<tr>
					<th>定时发布<input type="checkbox" name="shijian"  value='T'>:</th>
					<td >
		...
		<table  id="labelTable"  class="input tabContent" style="display: table;">
			<tbody>
				...
				<td>
					<select id="asas" name="name" onchange="aa()" >
						<option value="1" <c:if test="${'1' eq asas}"></c:if> >[图书音像]</option>
					</select>
					<select id="categorySelect" name="leimu" onchange="bb()">
						<option value="1" <c:if test="${'1' eq categorySelect}"></c:if> >[国外大片]</option>
					</select>
					<select id="childSelect" >
						<option value="1" <c:if test="${'1' eq childSelect}"></c:if> >[钢铁侠]</option>
					</select>
				</td>
			</tr>
			<tr>
				<th>所属频道:</th>
				<td>
					<select id="channelSelect" name="pindao" value="${requestScope.wz.pindao}">
					<option value="1" <c:if test="${'1' eq channelSelect}"></c:if> >游戏频道</option>
						<option value="3" >[yy直播]</option>
						<option value="4" >[探险频道]</option>
						<option value="5" >[国外频道]</option>
					</select>
				</td>
			</tr>
			<tr>
				<th>所属品牌:</th>
				<td>
					<select id="brandSelect" name="pinpai" value="${requestScope.wz.pinpai}">
					
					<option value="1" <c:if test="${'1' eq brandSelect}"></c:if> >苹果</option>
						<option value="2" >[爱国者]</option>
			...
			<th>文章标签:</th>
			<td>
			<input type="text" name="leibie" class="text label" value="${requestScope.wz.leibie}" ></td>
			<td><a href="javascript:;" class="deleteProductImage">[删除]</a></td></tr>
		...
		<table id="productImageTable" class="input tabContent" style="diplay:none;">
			<tr>
				...
				<td>
				<img height='50px' width='50px' src='../upload/${requestScope.wz.headimg}'/>
				</td>
				<td>
		...
		<table class="input tabContent" style="display:none;">
			<tr>	
				<td>
					<textarea id="editor" name="content" class="editor" style="width: 100%; display: none;">	
						${requestScope.wz.content}
					</textarea>
				</td>
			</tr>
		</table>
